...
public class DoctorController {
	...
	public ResponseEntity<?> createDoctor(@RequestBody Doctor doctor, UriComponentsBuilder ucBuilder) {
		logger.info("Creating User Type : {}", doctor);

		if (doctorRepository.exists(doctor.getId())) {
			logger.error("Unable to addDoctor. A Doctor with name {} already exist", doctor.getName());
			return new ResponseEntity(
					new CustomErrorType(
							"Unable to create. A Doctor with name " + doctor.getName() + " already exist."),
					HttpStatus.CONFLICT);
		}
	...
	public ResponseEntity<?> updateDoctor(@PathVariable("id") int id, @RequestBody Doctor doctor1) {
		...
		if (doctor1 == null) {
			...
			return new ResponseEntity(new CustomErrorType("Unable to upate. Doctor with id " + id + " not found."),
					HttpStatus.NOT_FOUND);
		}

		doctor.setContactNo(doctor1.getContactNo());
