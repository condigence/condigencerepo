                    ...
                    ngModel.$render = function () {
                       /* selectedDate = ngModel.$viewValue
						...
						if(ngModel.$viewValue){
							if($scope.parseDate){
								selectedDate = $scope.parseDate(ngModel.$viewValue)
							}
							else{
								//selectedDate = new Date(ngModel.$viewValue)
								var tempFifo = '';
								tempFifo = ngModel.$viewValue.split('/')
								selectedDate = new Date(tempFifo[1]+'/'+tempFifo[0]+'/'+tempFifo[2])
							}							
						}
						else{
							selectedDate = null;
